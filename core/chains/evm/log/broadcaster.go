...
func (r registration) isSubscribed() {
	close(r.subscribed)
}

func (r registration) awaitSubscription() {
}

func (r registration) isRemoved() {
	close(r.removed)
}

func (r registration) awaitRemoval() {
	t := time.NewTimer(subWaitSanityLimit)
	defer t.Stop()
	select {
	case <-r.removed:
	case <-t.C:
		panic("awaiting registration removal timed out")
	}
}
...
func (b *broadcaster) Register(listener Listener, opts ListenerOpts) (unsubscribe func()) {
	...
	reg := registration{listener, opts, chSubbed, chRemoved}
